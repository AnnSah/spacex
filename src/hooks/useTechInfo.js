const useTexhInfo = ({ name, rocket, engine, isEngine }) => {
    const header = name.toUpperCase();

    const firstRow = {
        title: isEngine ? "NUMBER" : "HEIGHT",
        value: isEngine ? engine.number : rocket.height.meters + "m/" + rocket.height.feet + "/f",
    ...
    const thirdRow = {
        title: isEngine ? "PROPELLANT2" : "DIAMETER",
    ...
    const body = [firstRow, secondRow, thirdRow, forthRow];
